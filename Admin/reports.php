...
$totalStudents    = safeCount($conn, 'Users') - 1;
$totalQuestions   = safeCount($conn, 'Questions');
$totalCareers     = safeCount($conn, 'Careers');
$totalAssessments = safeCount($conn, 'Assessments');

// Top recommended careers
$topCareers = null;
$check = mysqli_query($conn, "SHOW TABLES LIKE 'Recommendations'");
if ($check && mysqli_num_rows($check) > 0) {
    $topCareers = mysqli_query($conn,
        "SELECT c.Title as career_title, COUNT(*) total
...
// Recent assessment results – if Assessments and Recommendations exist
$recentResults = null;
$check2 = mysqli_query($conn, "SHOW TABLES LIKE 'Assessments'");
if ($check2 && mysqli_num_rows($check2) > 0) {
    $recentResults = mysqli_query($conn,
        "SELECT a.Date as created_at, u.first_name, u.last_name, u.email,
                c.Title as career_title, r.MatchScore as score
         FROM Assessments a
         JOIN Users u ON a.UserID = u.id
         LEFT JOIN Recommendations r ON r.AssessmentID = a.AssessmentID
         LEFT JOIN Careers c ON c.CareerID = r.CareerID
...
<div class="admin-main">
  ...
  <div class="admin-content">
    ...
    <!-- Stats -->
    <div class="stat-grid">
      <div class="stat-card"><div class="stat-icon blue"><i class="fa-solid fa-user-graduate"></i></div><div class="stat-info"><span class="stat-number"><?= $totalStudents ?></span><span class="stat-label">Total Students</span></div></div>
      <div class="stat-card"><div class="stat-icon teal"><i class="fa-solid fa-chart-line"></i></div><div class="stat-info"><span class="stat-number"><?= $totalAssessments ?></span><span class="stat-label">Assessments Taken</span></div></div>
      <div class="stat-card"><div class="stat-icon purple"><i class="fa-solid fa-briefcase"></i></div><div class="stat-info"><span class="stat-number"><?= $totalCareers ?></span><span class="stat-label">Career Paths</span></div></div>
      <div class="stat-card"><div class="stat-icon green"><i class="fa-solid fa-clipboard-question"></i></div><div class="stat-info"><span class="stat-number"><?= $totalQuestions ?></span><span class="stat-label">Questions</span></div></div>
    </div>
